...
def main():
    ...
    parser.add_argument(
        "--data_dir", type=str, help="data directory containing the *_meta.csv"
    )
    ...
    train_dataloader = DataLoader(
        train_set,
        batch_size=hps.TrainToMany.train_batch_size,
    ...
    test_dataloader = DataLoader(
        test_set,
        batch_size=hps.TrainToMany.test_batch_size,
        shuffle=hps.TrainToMany.shuffle,
        num_workers=hps.TrainToMany.num_workers,
        collate_fn=collate_fn,
    )
    ...
    for epoch in range(hps.TrainToMany.epochs):
        ...
        for idx, batch in enumerate(train_dataloader):
            ...
            loss = masked_mse_loss(
                outputs.transpose(0, 1), target_mels.transpose(0, 1), lengths
            )
            loss.backward()
            optimizer.step()
            ...
            if idx % 1 == 0:  # print every batch
                print(
                    "[%d, %5d] Training loss: %.5f" % (epoch + 1, idx + 1, running_loss)
                )
                running_loss = 0.0
        # save model parameters
        torch.save(
            model.state_dict(),
            os.path.join(args.model_dir, "bnf-vc-to-many-{}.pt".format(epoch)),
        )
        # validation
        print("Validating...")
        model.eval()
        ...
        for dev_batch in dev_dataloader:
            dev_inputs = torch.cat([dev_batch["bnf"], dev_batch["f0"]], dim=2).to(
                device
            )
            ...
            # run backward pass
            dev_loss = masked_mse_loss(
                dev_outputs.transpose(0, 1),
                dev_target_mels.transpose(0, 1),
                dev_lengths,
            )
            dev_running_loss += dev_loss
        print(
            "[%d] Validation loss: %.5f"
        ...
        for test_batch in test_dataloader:
            print("Testing...")
            test_inputs = torch.cat([test_batch["bnf"], test_batch["f0"]], dim=2).to(
                device
            )
            ...
            draw_melspectrograms(
                args.test_dir,
                step=epoch,
                mel_batch=test_outputs.cpu().detach().numpy(),
                mel_lengths=test_batch["length"].numpy(),
                ids=test_batch["fid"],
                prefix="predicted",
            )
            draw_melspectrograms(
                args.test_dir,
                step=epoch,
                mel_batch=test_target_mels.numpy(),
                mel_lengths=test_batch["length"].numpy(),
                ids=test_batch["fid"],
                prefix="groundtruth",
            )
