...
def wav2unnormalized_mfcc(
    ...
    power_mel = _power_spec2power_mel(
        power_spec, sr=sr, n_fft=n_fft, num_mels=num_mels, fmin=fmin, fmax=fmax
    )
...
def normalized_db_mel2wav(
    ...
    min_db=hparams["min_db"],
    center=hparams["center"],
    griffin_lim_power=hparams["griffin_lim_power"],
    ...
    power_spec = _power_mel2power_spec(
        power_mel, sr=sr, n_fft=n_fft, num_mels=num_mels, fmin=fmin, fmax=fmax
    )  # 矩阵求逆猜出来的spec
    magnitude_spec = power_spec**0.5  # (time, n_fft/2+1)
    # print('-----1:', magnitude_spec.shape)
    # magnitude_spec_t = magnitude_spec.T
    griffinlim_powered_magnitude_spec = (
        magnitude_spec**griffin_lim_power
    )  # (time, n_fft/2+1)
    # print('-----2:', griffinlim_powered_magnitude_spec.shape)
    # 送入griffinlim的是正常的 (time, n_fft/2+1)
    emph_wav_arr = _griffin_lim(
        griffinlim_powered_magnitude_spec,
        gl_iterations=griffin_lim_iterations,
        n_fft=n_fft,
        hop_len=hop_len,
        win_len=win_len,
        window=window,
...
def _istft(stft_matrix, hop_len, win_len, window):
    return librosa.core.istft(
        stft_matrix, hop_length=hop_len, win_length=win_len, window=window
    )
...
def _power_spec2power_mel(power_spec, sr, n_fft, num_mels, fmin, fmax):
    ...
    global _mel_basis
    _mel_basis = (
        librosa.filters.mel(sr, n_fft, n_mels=num_mels, fmin=fmin, fmax=fmax)
...
def _power_mel2power_spec(power_mel, sr, n_fft, num_mels, fmin, fmax):
    ...
    _inv_mel_basis = (
        np.linalg.pinv(_mel_basis) if _inv_mel_basis is None else _inv_mel_basis
    )
...
def _power2db(power_spec, ref_db, tol=1e-5):
    return 10.0 * np.log10(power_spec + tol) - ref_db
...
def _db_normalize(db, min_db):
    return np.clip((db - min_db) / -min_db, 0.0, 1.0)


# inv操作
# 超参数个数：1
def _db_denormalize(normalized_db, min_db):
    return np.clip(normalized_db, 0.0, 1.0) * -min_db + min_db
...
def _griffin_lim(
    magnitude_spec, gl_iterations, n_fft, hop_len, win_len, window, center
):
    ...
    for _i in range(gl_iterations):
        ...
        y = _istft(
            complex_mag * angles, hop_len=hop_len, win_len=win_len, window=window
        )
    return y
