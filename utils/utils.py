...
def masked_mse_loss(inputs: torch.Tensor, targets: torch.Tensor, lengths: torch.Tensor):
    ...
    if lengths is None:
        ...
        mask = torch.arange(max_len).expand([len(lengths), max_len]).to(
            device
        ) < lengths.unsqueeze(1)
        mask = mask.to(dtype=torch.float32)
        mse_loss = torch.mean(
            torch.sum(torch.mean((inputs - targets) ** 2, dim=2) * mask, dim=1)
...
class F0Extractor:
    def __init__(
        self, method_type, sample_rate=16000, hop_time=0.01, min_f0=20, max_f0=600
    ):
        """FO EXTRACTOR
    ...
    def _extract_vuv(signal, unvoiced_value):
        is_unvoiced = np.isclose(
            signal, unvoiced_value * np.ones_like(signal), atol=1e-2
        )
            ...
            if is_voiced[i]:
                if not in_voiced_region:
                    ...
                    # includes the bordering voiced frames.
                        ...
                        interpolated_region = np.linspace(
                            start_voiced_value, end_voiced_value, unvoiced_region_length
                        )
