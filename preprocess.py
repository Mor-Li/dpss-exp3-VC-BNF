...
def main():
    ...
    for i, wav_f in tqdm(enumerate(wav_files)):
        ...
        mel_spec, linear_spec, f0, BNFs = length_validate((
            mel_spec,
            linear_spec,
            f0,
            BNFs,
        ...
        if i < dev_start_idx:
            ...
            with open(
                os.path.join(args.save_dir, "train_meta.csv"), "a", encoding="utf-8"
            ) as train_f:
                train_f.write(
                    "{}|BNFs/{}.npy|mels/{}.npy|linears/{}.npy|f0s/{}.npy\n".format(
                        fid, fid, fid, fid, fid
                    )
                )
            ...
            dev_set.append(fid)
            with open(
                os.path.join(args.save_dir, "dev_meta.csv"), "a", encoding="utf-8"
            ) as dev_f:
                dev_f.write(
                    "{}|BNFs/{}.npy|mels/{}.npy|linears/{}.npy|f0s/{}.npy\n".format(
                        fid, fid, fid, fid, fid
            ...
            test_set.append(fid)
            with open(
                os.path.join(args.save_dir, "test_meta.csv"), "a", encoding="utf-8"
            ) as test_f:
                test_f.write(
                    "{}|BNFs/{}.npy|mels/{}.npy|linears/{}.npy|f0s/{}.npy\n".format(
                        fid, fid, fid, fid, fid
